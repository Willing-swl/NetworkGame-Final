...
public class PlayerRoamingController : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        reloadAction.action.performed += _ => OnReload();// 重新装填？
        jumpAction.action.performed += _ => OnJump();// 跳跃
        dodgeAction.action.performed += _ => OnDodge();// 闪避
        fireAction.action.performed += _ => OnFire(); //喷射
    }
